...
def build_pdf(positions: list) -> bytes:
    ...
    bc.categoryAxis.labels.textColor = colors.HexColor("#4a6080")
    bc.valueAxis.labels.fontName     = "Helvetica"
    bc.valueAxis.labels.fontSize     = 7
    bc.valueAxis.labels.textColor    = colors.HexColor("#4a6080")
    bc.valueAxis.strokeColor         = colors.HexColor("#1e2d4a")
    bc.categoryAxis.strokeColor      = colors.HexColor("#1e2d4a")

    # สีแต่ละแท่ง
    for i, p in enumerate(sorted_pos):
        bc.bars[0].fillColor = C_GREEN if p["pnl"] >= 0 else C_RED
